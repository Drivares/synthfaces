...
@click.option('truncation_psi', '--trunc', help='Truncation psi', type=float, default=None, metavar='FLOAT', show_default=True)
...
def calc_metrics(ctx, truncation_psi, network_pkl, metrics, data, mirror, gpus, verbose):
    """Calculate quality metrics for previous training run or pretrained network pickle.
    Examples:
    \b
    ...
    if data is not None:
        args.dataset_kwargs = dnnlib.EasyDict(class_name='core.training.dataset.ImageFolderDataset', path=data)
    ...
    args.dataset_kwargs.use_labels = bool(args.G.c_dim != 0)
