    ...
    def image_summary(self, image_set, step, phase='train', save_val=True):
        ...
        if CONFIG.local_rank == 0:
            for tag, image_numpy in image_set.items():
                ...
                image_numpy = cv2.resize(image_numpy, None, fx=360.0/min(h,w), fy=360.0/min(h,w), interpolation=cv2.INTER_NEAREST)
...
def normalize_image(image, mode=0):
    """
    ...
    if mode == 0:
        ...
        image_min = image_flat.min(dim=1, keepdim=False)[0].view(3,1,1)
        image_max = image_flat.max(dim=1, keepdim=False)[0].view(3,1,1)
