...
class ResNet_D_Dec(nn.Module):
    ...
    def __init__(self, block, layers, out_channel=1, norm_layer=None, large_kernel=False, late_downsample=False):
        ...
        self.refine_OS1 = nn.Sequential(
            ...
            norm_layer(32),
            self.leaky_relu,
            nn.Conv2d(32, out_channel, kernel_size=self.kernel_size, stride=1, padding=self.kernel_size//2),)
...
class ResNet_D_Dec_Color(nn.Module):

    def __init__(self, block, layers, out_channel=1, norm_layer=None, large_kernel=False, late_downsample=False):
        super(ResNet_D_Dec_Color, self).__init__()
        self.logger = logging.getLogger("Logger")
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
        ...
        self.color_OS1 = nn.Sequential(
            nn.Conv2d(32, 32, kernel_size=self.kernel_size, stride=1, padding=self.kernel_size//2, bias=False),
            norm_layer(32),
            self.leaky_relu,
            nn.Conv2d(32, out_channel*3, kernel_size=self.kernel_size, stride=1, padding=self.kernel_size//2),)
        
        self.color_OS4 = nn.Sequential(
            nn.Conv2d(64, 32, kernel_size=self.kernel_size, stride=1, padding=self.kernel_size//2, bias=False),
            norm_layer(32),
            self.leaky_relu,
            nn.Conv2d(32, out_channel*3, kernel_size=self.kernel_size, stride=1, padding=self.kernel_size//2),)

        self.color_OS8 = nn.Sequential(
            nn.Conv2d(128, 32, kernel_size=self.kernel_size, stride=1, padding=self.kernel_size//2, bias=False),
            norm_layer(32),
            self.leaky_relu,
            nn.Conv2d(32, out_channel*3, kernel_size=self.kernel_size, stride=1, padding=self.kernel_size//2),)
